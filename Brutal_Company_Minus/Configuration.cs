...
namespace Brutal_Company_Minus
{
    internal class Configuration
    {
        ...
        {
            // Create config file
            config = new ConfigFile(Paths.ConfigPath + "\\Brutal_Company_Minus.cfg", true);
            ...
            foreach (Event e in Plugin.events)
            {
                ...
                foreach (KeyValuePair<ScaleType, Scale> obj in e.ScaleList)
                {
                    ...
                    baseScales.Add(config.Bind(e.Name(), obj.Key.ToString() + " Base Scale Value", obj.Value.BaseScale, "Starting Value"));
                    incrementScales.Add(config.Bind(e.Name(), obj.Key.ToString() + " Increment Scale Value", obj.Value.IncrementScale, "Formula: BaseScale + (IncrementScale * DaysPassed)"));
                }
