...
namespace Brutal_Company_Minus.Patches
{
    ...
    internal class SpringManAIPatch
    {
        ...
        static IEnumerable<CodeInstruction> OnUpdateIL(IEnumerable<CodeInstruction> instructions, ILGenerator il)
        {
            ...
            int firstindex = -1;
            CodeInstruction firstFlag = Transpilers.EmitDelegate<Func<bool>>(() => Server.Instance.isAntiCoilHead.Value);

            int secondindex = -1;
            CodeInstruction secondFlag = Transpilers.EmitDelegate<Func<bool>>(() => !Server.Instance.isAntiCoilHead.Value);
